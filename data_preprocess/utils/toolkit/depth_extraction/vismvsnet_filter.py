import sys
sys.path.append('.')
sys.path.append('submodules/Relightable3DGaussian/vismvsnet')
import os
import cv2
import torch
import argparse
...
from submodules.Relightable3DGaussian.vismvsnet.core.nn_utils import bin_op_reduce
from submodules.Relightable3DGaussian.vismvsnet.utils.preproc import recursive_apply
from submodules.Relightable3DGaussian.vismvsnet.core.homography import get_pixel_grids
from submodules.Relightable3DGaussian.vismvsnet.utils.io_utils import load_cam, load_pfm
...
if __name__ == '__main__':
    ...
    parser.add_argument('--out_shape', type=str, default='1600,1200', help='The size of the output one.')
    
    args = parser.parse_args()

    pthresh = [float(v) for v in args.pthresh.split(',')]
    
    pair = load_pair(args.pair, min_views=None)
    n_views = len(pair['id_list'])
    out_shape = tuple([int(v) for v in args.out_shape.split(',')])
    ...
    for i, id in tqdm(enumerate(pair['id_list']), 'load data', n_views):
        image = load_image(f'{args.data}/{file_names[i]}.jpg').transpose(2,0,1)
    ...
    for i, id in enumerate(pair['id_list']):
        ...
        depth = cv2.resize(depth, out_shape)

        mask = mask.astype(np.uint8)*255
        mask = cv2.resize(mask, dsize=None, fx=2, fy=2, interpolation=cv2.INTER_NEAREST)
        mask = cv2.resize(mask, out_shape)
        
        np.save(f"{out_depth_dir}/{file_names[i]}.npy", depth)
        save_image(f"{out_mask_dir}/{file_names[i]}.png", mask)
